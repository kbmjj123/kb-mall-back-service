...
export default {
	...
	*/
	...
	/**
	 * 根据购物车生成的临时分享id来生成对应的token
	*/
	generateShareCarToken: (id: string) => {
		return jwt.sign({id}, process.env.JWT_SHOPPING_CAR_SECRET as string, { expiresIn: Number(process.env.JWT_SHOPPING_CAR_EXPIRES_IN_TIME) })
	},
	/**
	 * 根据token，获取对应的生成的临时分享出来的购物车商品清单信息
	*/
	validateShoppingCarToken: (token: string) => {
		try{
			const decodeInfo = jwt.verify(token, process.env.JWT_SHOPPING_CAR_SECRET as string) as JwtPayload
			if(decodeInfo.id){
				return decodeInfo.id
			}else{
				return null
