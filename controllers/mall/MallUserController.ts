...
@Route('/api/user')
@Tags('商城/用户模块')
export class MallUserController extends BaseController {

	/**
	 * 用户登录
	*/
	@Post("/login")
	public async login(@Request() req: ExpressRequest, @Body() requestBody: UserLoginParams): Promise<BaseObjectEntity<UserDTO>> {
		const res = req.res
